...
CHECKS='check_installs check_deps_resolve check_cli_runs check_ships_disabled check_refuses_without_key check_service_command_runs check_key_not_in_argv check_key_not_in_uci check_key_not_in_procd_env check_config_survives check_clean_removal'
...
out=$(/usr/bin/hermes --version 2>&1) || { echo "$out"; fail "[3/11] check_cli_runs" "hermes --version exited non-zero"; }
echo "$out" | grep -q "Hermes Agent" || { echo "$out"; fail "[3/11] check_cli_runs" "unexpected output"; }
echo "PASS [3/11] check_cli_runs"

# ---- 4. ships disabled, and says so instead of erroring ----
[ -e /etc/rc.d/S95hermes-agent ] || fail "[4/11] check_ships_disabled" "post-install did not enable the service"
msg=$(/etc/init.d/hermes-agent start 2>&1 || true)
echo "$msg" | grep -q "disabled in /etc/config/hermes" || { echo "$msg"; fail "[4/11] check_ships_disabled" "starting an unconfigured service did not say why"; }
pgrep -f "hermes_cli/main.py gateway" >/dev/null 2>&1 && fail "[4/11] check_ships_disabled" "it started anyway"
echo "PASS [4/11] check_ships_disabled"

# ---- 5. refuses without a key, naming the fix ----
uci set hermes.main.enabled=1 >/dev/null 2>&1; uci commit hermes
msg=$(/etc/init.d/hermes-agent start 2>&1 || true)
echo "$msg" | grep -q "no API key" || { echo "$msg"; fail "[5/11] check_refuses_without_key" "did not refuse"; }
echo "$msg" | grep -q "provider.key" || { echo "$msg"; fail "[5/11] check_refuses_without_key" "refused without naming the file to write"; }
echo "PASS [5/11] check_refuses_without_key"
...
start_service
STUB
sh /tmp/fakeprocd.sh >/tmp/fp.log 2>&1 || { cat /tmp/fp.log; fail "[6/11] check_service_command_runs" "start_service did not complete"; }
...
if kill -0 "$GW" 2>/dev/null; then
	...
	if tr '\0' '\n' < "/proc/$GW/cmdline" | grep -q "$SECRET"; then
		kill "$GW" 2>/dev/null; fail "[7/11] check_key_not_in_argv" "the key is in the process command line"
	fi
	echo "PASS [7/11] check_key_not_in_argv"
	...
	fail "[7/11] check_key_not_in_argv" "the gateway exited, so nothing was inspected"
fi

uci show hermes 2>/dev/null | grep -q "$SECRET" && fail "[8/11] check_key_not_in_uci" "the key is in UCI"
echo "PASS [8/11] check_key_not_in_uci"
...
if [ -s /tmp/envv ]; then
	if grep -q "$SECRET" /tmp/envv; then
		fail "[9/11] check_key_not_in_procd_env" "the key is in the procd env block, so ubus call service list exposes it"
	fi
	echo "PASS [9/11] check_key_not_in_procd_env"
else
	fail "[9/11] check_key_not_in_procd_env" "no env block was captured, so this check measured nothing"
fi
...
grep -qF "$marker" /etc/config/hermes || fail "[10/11] check_config_survives" "the config was overwritten by a reinstall"
